# 互联网白嫖指南

## GitHub action （一个免费使用的自动化工具）

[https://github.com/](https://github.com/)

## vercel （Vercel 是一个云平台，专为前端开发者和微服务设计。它提供了一种简单的方式来部署你的 JavaScript、TypeScript、React、Vue、Angular 等项目。）

[https://vercel.com/](https://vercel.com/)

## cloudflare（免费CDN）

[https://cloudflare.com/](https://cloudflare.com/)

## 雨云（签到免费领服务器）

[https://www.rainyun.com/](https://www.rainyun.com/baize_)

## eu.org（免费的eu.org二级域名）

[https://nic.eu.org/](https://nic.eu.org/)

## netlify（和vercel类型相似）

[https://www.netlify.com/](https://www.netlify.com/)

## 七牛云（免费CDN，图床）

[https://www.qiniu.com/products/qcdn](https://www.qiniu.com/products/qcdn)

## 又拍云（免费CDN，图床）

[https://www.upyun.com/products/cdn](https://www.upyun.com/products/cdn)

## jsdelivr（jsDelivr 是一个免费的公开 CDN，它允许开发者和网站所有者在任何地方快速托管 JavaScript 库、jQuery 插件、CSS 框架、字体文件等前端资源）

[https://www.jsdelivr.com/](https://www.jsdelivr.com/)

## 文心一言（免费的中文ai，效果不错）

[https://yiyan.baidu.com/welcome](https://yiyan.baidu.com/welcome)

## imgix（免费的图片CDN）

[https://imgix.com/](https://imgix.com/)

## epic（著名的游戏慈善平台）

[https://www.epicgames.com/](https://www.epicgames.com/)

## iconfont（免费图标库）

[https://www.iconfont.cn/](https://www.iconfont.cn/)

## oracle free tier（免费服务器，很难注册）

[https://www.oracle.com/cloud/free/](https://www.oracle.com/cloud/free/)

## aws cloudfront（亚马逊免费CDN,每月1T流量，有香港节点，速度很快）

[https://aws.amazon.com/cn/cloudfront/](https://aws.amazon.com/cn/cloudfront/)

## freenom（提供免费的顶级域名）

[https://www.freenom.com/](https://www.freenom.com/)

## unpkg（unpkg 是一个快速、全球内容分发网络（CDN），它可以为 npm 的所有包提供即时的文件访问。）

[https://unpkg.com/](https://unpkg.com/)

## azure（来自微软的免费服务器）

[https://azure.microsoft.com/zh-cn/free/](https://azure.microsoft.com/zh-cn/free/)
## buypass(长达180天的免费ssl证书，但国内无法访问)
[https://www.buypass.com/](https://www.buypass.com/)
## etsencrypt（著名SSL证书提供商，SSL证书有效期90天）
[https://letsencrypt.org/zh-cn/](https://letsencrypt.org/zh-cn/)