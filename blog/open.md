
# 推荐一下使用过不错的开源产品
## teamIDE（Team IDE 工具 集成MySql、Oracle、金仓、达梦、神通等数据库、SSH、FTP、Redis、Zookeeper、Kafka、Elasticsearch等管理工具）
[https://github.com/team-ide/teamide](https://github.com/team-ide/teamide)
## 1panel（现代化、开源的 Linux 服务器运维管理面板）
[https://github.com/1Panel-dev/1Panel](https://github.com/1Panel-dev/1Panel)
## ttime（一款简洁、高效、好看的输入、截图、单词翻译软件）
[https://github.com/InkTimeRecord/TTime](https://github.com/InkTimeRecord/TTime)
## Oracle VM VirtualBox（开源虚拟机）
[https://www.virtualbox.org/](https://www.virtualbox.org/)
## xterminal（好看实用的ssh工具）
[https://www.xterminal.cn/](https://www.xterminal.cn/)
## Motrix（堪比IDE的下载器）
[https://github.com/agalwood/Motrix](https://github.com/agalwood/Motrix)
## listen1（免费听歌软件）
[https://github.com/listen1/listen1_desktop](https://github.com/listen1/listen1_desktop)
## PicList（一款基于PicGo的图片上传和管理工具，比picgo好用）
[https://github.com/Kuingsmile/PicList](https://github.com/Kuingsmile/PicList)
## SteamTools（「Watt Toolkit」是一个开源跨平台的多功能 Steam 工具箱）
[https://github.com/BeyondDimension/SteamTools](https://github.com/BeyondDimension/SteamTools)
## spider-flow（可视化爬虫）
[https://github.com/ssssssss-team/spider-flow](https://github.com/ssssssss-team/spider-flow)
## stable-diffusion（AI绘图）
[https://github.com/AUTOMATIC1111/stable-diffusion-webui](https://github.com/AUTOMATIC1111/stable-diffusion-webui)
## gpt_academic（学术版GPT）
[https://github.com/binary-husky/gpt_academic](https://github.com/binary-husky/gpt_academic)
## TrafficMonitor（这是一个用于显示当前网速、CPU及内存利用率的桌面悬浮窗软件）
[https://github.com/zhongyang219/TrafficMonitor](https://github.com/zhongyang219/TrafficMonitor)
## GitHub520（让你“爱”上 GitHub，解决访问时图裂、加载慢的问题）
[https://github.com/521xueweihan/GitHub520](https://github.com/521xueweihan/GitHub520)
## vimcdoc（Vim 中文文档计划）
[https://github.com/yianwillis/vimcdoc](https://github.com/yianwillis/vimcdoc)
## reference（reference）
[https://github.com/jaywcjlove/reference](https://github.com/jaywcjlove/reference)
## 996ICU（996ICU）
[https://github.com/995icu/996ICU](https://github.com/995icu/996ICU)
## VPet（虚拟桌宠模拟器 一个开源的桌宠软件, 可以内置到任何WPF应用程序）
[https://github.com/LorisYounger/VPet](https://github.com/LorisYounger/VPet)
## Data-Structure（《数据结构》-严蔚敏.吴伟民-教材源码与习题解析）
[https://github.com/kangjianwei/Data-Structure](https://github.com/kangjianwei/Data-Structure)
## github-chinese（GitHub 汉化插件，GitHub 中文化界面）
[https://greasyfork.org/scripts/](https://greasyfork.org/scripts/435208-github-%E4%B8%AD%E6%96%87%E5%8C%96%E6%8F%92%E4%BB%B6/code/GitHub%20%E4%B8%AD%E6%96%87%E5%8C%96%E6%8F%92%E4%BB%B6.user.js)
## developer-roadmap（交互式路线图、指南和其他教育内容可帮助开发人员在职业生涯中成长）
[https://github.com/kamranahmedse/developer-roadmap](https://github.com/kamranahmedse/developer-roadmap)
## gpt4free-ts（提供免费的 OpenAI GPT-4 API）
[https://github.com/xiangsx/gpt4free-ts/](https://github.com/xiangsx/gpt4free-ts/)
## it-tools（开发工具箱）
[https://github.com/CorentinTh/it-tools](https://github.com/CorentinTh/it-tools)
## nerd-fonts（开源字体）
[https://github.com/ryanoasis/nerd-fonts](https://github.com/ryanoasis/nerd-fonts)
## wsl-manager（用于快速管理 WSL2 实例的 GUI）
https://github.com/bostrot/wsl2-distro-manager[](https://github.com/bostrot/wsl2-distro-manager)